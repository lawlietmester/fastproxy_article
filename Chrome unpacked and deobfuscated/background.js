...
function SOI() {
    ...
    DPE(), C({
        url: "common/oninstalled.html"
    })
}

function SOU() {
    DPE(), C({
        url: "common/onupdated.html"
    })
}
...
function start() {
    pc = 0, cc = 0, chrome.proxy.settings.get({
        ...
        if (a && ("controllable_by_this_extension" == a.levelOfControl || "controlled_by_this_extension" == a.levelOfControl) && navigator && navigator.onLine && -1 != navigator.userAgent.indexOf("Chrome")) LC();
        else var b = setTimeout(() => {
